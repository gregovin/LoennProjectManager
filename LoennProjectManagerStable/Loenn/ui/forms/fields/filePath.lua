...
local function buttonPressed(self, extension, location,require_dir)
...
function directFilepathField.getElement(name, value, options)
    ...
    local button = uiElements.button("", buttonPressed(formField, options.extension or "bin",options.location,options.require_dir)):with({
        minWidth = minWidth,
        maxWidth = maxWidth
    })
