...
function script.run(args)
    ...
    local addTileset = function()
        local success, logMessage,displayMessage = tilesetHandler.addTileset(path,args.name,copyMask,args.sound,pUtils.setAsList(args.ignores),templateInfo,args.customMask,false,target)
