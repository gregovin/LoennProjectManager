...
function postscript.run(args)
    ...
    local forward = function()
        local success, message =tilesetHandler.editTileset(selTilesetName,false,args.sound,args.ignores,copyMask,templateInfo,args.customMask,target)
        if not success then
    ...
    local backward = function()
        local success, message = tilesetHandler.editTileset(selTilesetName,false,tilesetDetails.sound,tilesetDetails.ignores,tilesetDetails.copy,tilesetDetails.templateInfo or "",tilesetDetails.masks or "",target)
        if not success then
            logging.warning(string.format("Failed to write to %s due to the following error:\n%s",target,message))
        end
        celesteRenderer.loadCustomTilesetAutotiler(state)
