...
local script = {
    ...
    tooltips = {
        campaignName =
        "The name for your campaign. Select from the list or enter a new entry to create a new campaign. \n Must be a valid portable filename",
    },
    parameters = {
        campaignName = "",
    },
...
function script.run(args)
    ...
    if projectDetails.projectName and projectDetails.username then
        local target = fileSystem.joinpath(modsDir, projectDetails.projectName, "Maps", projectDetails.username,
            args.campaignName)
        if $(projectDetails.campaigns):contains(args.campaignName) then
            projectLoader.loadCampaign(target)
        else
            projectLoader.newCampaign(target, projectDetails)
        end
