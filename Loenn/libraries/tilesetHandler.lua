...
function handler.prepareTilesetPath(projectDetails)
    ...
    if not fileSystem.isDirectory(fileSystem.joinpath(tilesetsDir,path)) then
        ...
        if not success then
            logging.warning(string.format("Failed to create tilesets folder at %s for project %s due to the following error:\n%s",fileSystem.joinpath(tilesetsDir,path),projectDetails.name,message))
...
function handler.updateCampaignMetadata(projectDetails,state,foreground,path)
    local clocal = fileSystem.joinpath(modsDir,projectDetails.name,"Maps",projectDetails.username,projectDetails.campaign)
